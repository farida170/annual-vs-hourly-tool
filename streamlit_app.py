...
CI_CMAP = LinearSegmentedColormap.from_list(
    ...
    [
        (0.00, "#02590F"),   # low CI = good
        (MID_POS, "#FFFFFF"),
        (1.00, "#000000"),   # high CI = bad
    ],
)

# Price colormap similar to your red/yellow/green diverging plot
PRICE_CMAP = plt.get_cmap("RdYlGn_r")  # green for low/negative, red for high
...
def load_all():
    ...
    # Ember price timestamps: naive -> localize UTC
...
def build_df_for_year(year, demand, ci, wind, solar, grid, price):
    ...
    if len(demand_y) == 0 or len(ci_y) == 0 or len(wind_y) == 0 or len(solar_y) == 0 or len(grid_y) == 0:
    ...
    df = (
        ...
        .merge(price_y[["ts_utc", "Price (EUR/MWhe)"]].rename(columns={"Price (EUR/MWhe)": "price_eur_per_mwh"}), on="ts_utc", how="left")
        .sort_values("ts_utc")
...
def available_years(demand, ci, wind, solar, grid, price):
    ...
    for d, col in [(demand, "ts_utc"), (ci, "ts_utc"), (wind, "ts_utc"), (solar, "ts_utc"), (grid, "ts_utc"), (price, "ts_utc")]:
        ...
        sets.append(years)
    # Intersection of all required sources (ensures year works end-to-end)
    yrs = sorted(set.intersection(*sets)) if sets else []
    return yrs
...
if not years:
    st.error("No common years found across all datasets. Add more years or check timestamps.")
    st.stop()

st.sidebar.header("Year")
year = st.sidebar.selectbox("Select year", years, index=len(years)-1)

df, ci_note, _ = build_df_for_year(year, demand, ci, wind, solar, grid, price)
if df is None:
    st.error(f"Year {year} not available for all required series (demand/CI/CF/CFE).")
    st.stop()

st.caption(f"Year={year} | Hours={len(df)} | Carbon intensity conversion: {ci_note}")
...
# -----------------------------
# Solve BOTH: annual + hourly (side-by-side everywhere)
# -----------------------------
with st.spinner("Solving LPs (Annual and Hourly)…"):
    solA, errA = solve_procurement_lp(df, "annual", 1.0, allow_battery, TU)
    solH, errH = solve_procurement_lp(df, "hourly", x_target, allow_battery, TU)
...
with tab_heat:
    ...
    # CFE heatmap (0–1) with CFE-style colors
    ...
    # Carbon intensity heatmap (scale to percent-like visuals: use robust vmin/vmax)
